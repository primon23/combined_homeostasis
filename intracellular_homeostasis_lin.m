% intracellular_homeostasis_lin.m
%
...
%   In this version, CaMKIV activation is linear in calcium, which is
%   non-optimal for the dual controller, but allows the single controller
%   to better stabilize mean rate.
%
%   This code is used to produce Figure 7 in the paper,
...
%   CaMKIV activation, x2, depends linearly on calcium (x1)
...
for simulation_number = 1:2
    ...
    for i = 2:Nt;
        ...
        % x2 is e.g. CaMKIV activation.
        % In method-1 a linear filter of calcium
        % In method-2 an integration of calcium (alpha2=0)
        x2(i) = x2(i-1) + dt/tau2*(x1bar*(x1(i-1)/x1bar) - alpha2*x2(i-1) - x2_loss);
        x2(i) = max(x2(i),0);
